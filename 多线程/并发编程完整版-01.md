...
**1.同步阻塞**：比如用锅烧水，水开后，不会主动通知你。烧水开始执行后，需要一直等待水烧开。

**2.同步非阻塞**：比如用锅烧水，水开后，不会主动通知你。烧水开始执行后，不需要一直等待水烧开，可以去执行其他功能，但是需要时不时的查看水开了没。

**3.异步阻塞**：比如用水壶烧水，水开后，会主动通知你水烧开了。烧水开始执行后，需要一直等待水烧开。（FutureTask）

**4.异步非阻塞**：比如用水壶烧水，水开后，会主动通知你水烧开了。烧水开始执行后，不需要一直等待水烧开，可以去执行其他功能。

其中**异步非阻塞这个效果是最好的**，平时开发时，提升效率最好的方式就是采用异步非阻塞的方式处理一些多线程的任务。
...
* 每个Thread中都存储着一个成员变量，ThreadLocalMap (初次set()后Thread类创建这个map，后续当前线程所创建的ThreadLocal共用这个map)
...
* ThreadLocalMap的key是一个弱引用，弱引用的特点是，在GC时，也必须被回收。但弱引用仅能回收 key（`ThreadLocal` 对象），但 value 仍可能因线程长期存活而泄漏。必须显式调用 `remove()` 彻底清理。
...
### 3.3 happens-before（JVM层面的重排，不用记，印象即可）
...
public synchronized void method(){
    ...
    String localString = "This is a local string"; 	// 局部变量，不是共享资源
    System.out.println(localString); 				// 仅使用局部变量
}
```

Java 编译器在 JIT（Just-In-Time）编译过程中，通过逃逸分析（Escape Analysis）优化性能的一种方式。

- **JIT 编译器的优化**：编译器会发现 synchronized 关键字在这个方法中没有意义，因为不存在共享资源或者竞争条件。

- **锁消除**：JVM 在运行时会移除这个无意义的 synchronized 锁，从而提高代码执行效率。
...
![image.png](./并发编程完整版-01.assets/5960ce8eb65d44c299d542fea1b61781-4078842.png)

展开MarkWord

![image.png](https://gimg2.baidu.com/image_search/src=http%3A%2F%2Fimg-blog.csdnimg.cn%2F20200314202610433.png%3Fx-oss-process%3Dimage%2Fwatermark%2Ctype_ZmFuZ3poZW5naGVpdGk%2Cshadow_10%2Ctext_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L0ludHJvbmNoZW5n%2Csize_16%2Ccolor_FFFFFF%2Ct_70&refer=http%3A%2F%2Fimg-blog.csdnimg.cn&app=2002&size=f9999,10000&q=a80&n=0&g=0n&fmt=auto?sec=1659024071&t=5135a8e6d07b807c6bb771ceafd8a227)
...
| ------------------------------------------------------------ |
| ![image-20240929180110749](./并发编程完整版-01.assets/image-20240929180110749.png) |

整个锁升级状态的转变：

![image.png](./并发编程完整版-01.assets/20d71edbf3dd438b95358d0169b69226-4078971.png)

**Lock Record**以及**ObjectMonitor**存储的内容 - 这两个哪里来的？图中有解释：

![image.png](./并发编程完整版-01.assets/616fb837af0c4bcfac43e281d3d22e08-4078976-4078981.png)
...
* 实现原理是不一样，ReentrantLock基于`AQS`实现的，synchronized是基于`ObjectMonitor`
...
<img src="./并发编程完整版-01.assets/6f1f8f1f2ff4494c81adddef4a0897e6.png" alt="image.png" style="zoom:67%;" />
...
ReentrantLock 锁资源
哪个线程基于 CAS 将 state 从 0 修改为 1, 就代表拿到了锁资源
...
graph LR;
	head((head))
	tail((tail))
	Node1[Node <br> 互斥锁 <br> waitStatus = -1 <br> thread = null <br> 伪节点]
	Node2[Node <br> 互斥锁 <br> waitStatus = -1 <br> thread = B <br> B线程挂起]
	Node3[Node <br> 互斥锁 <br> waitStatus = 0 <br> thread = C <br> C线程挂起]
...
1.线程A先执行CAS，将state从0修改为1，线程A就获取到了锁资源，直接去执行业务代码即可
2.线程B再执行CAS，发现state已经是1了，无法获取到锁资源，所以需要排队等待资源（将自己封装为Node对象）
3.线程C再过来，是同样的步骤

排队过程：（新队列）

1. 将当前B线程的Node放到双向队列保存前，必须先有一个伪节点作为头结点（第一次保存到AQS中）

2. 然后再将B线程的Node挂在tail的后面（这样能确保是排在最后），并将B线程的Node上一个节点的状态修改为-1，再挂起B线程
...
* tryLock(time,unit);
  * 第一波分析，类似的代码：
    ...
    ```
  * 取消节点分析：![image.png](./并发编程完整版-01.assets/c283fb2473a8401985b9f874cb222f40.png)
...
graph LR;
	head((head))
	tail((tail))
	Node1[Node <br> 互斥锁 <br> waitStatus = -1 <br> thread = null]
	Node2[Node <br> 互斥锁 <br> waitStatus = 1 <br> thread = null]
	Node3[Node <br> 互斥锁 <br> waitStatus = 0 <br> thread = C]
...
线程A释放锁资源调用`unlock`方法,就是执行了`tryRelease`方法

1. 首先判断是不是线程A 持有着锁资源，如果不是就抛异常。
   - 如果是线程A 持有者锁资源，对 state - 1.
   - -1 成功后，会判断state是否为0，如果不是，方法结束。如果为 0，证明当前锁资源释放干净。
2. 查看头结点的状态是否不为0（判断是否为-1）。

   - 如果为0，代表后面没挂起的线程
   
   - 如果不为0，后续链表中有挂起的线程，需要唤醒。
   
3. 在唤醒线程时，需要先将当前的-1，改为0，找到有效节点唤醒。找到之后，唤醒线程即可
...
![image.png](./并发编程完整版-01.assets/09215181ecf44581aa9b0e464e05b418.png)
...
![image.png](./并发编程完整版-01.assets/08d73c23a8654fe2867269d63678cdee.png)
...
产生的4个必要条件：1. 互斥 2. 长持有 3. 不可剥夺 4. 环路
...
![image.png](./并发编程完整版-01.assets/b0cc492556d746a3a2c1ac9f31e06dde.png)
...
![image.png](./并发编程完整版-01.assets/be0f3126b64443d1bf1e05299f33d71a.png)
...
一定要以局部的方式去查看树结构的变化，他是从跟节点往下找较小的一个子节点，将较小的子节点挪动到父节点位置，再将循环往下走，如此一来，整个二叉堆的结构就可以保证了。
...
![image.png](./并发编程完整版-01.assets/c6fe53bff00f468f90440012c507ef2e.png)
...
![image.png](./并发编程完整版-01.assets/70d2778577b94910a15baad2778f0a9d.png)
