...
```java
 //订单详情
 List<OrderManageRespVO> records = resultPage.getRecords();
 log.info("开始CompletableFuture");
 List<CompletableFuture<OrderManageRespVO>> taskList = records.stream()
         .map(n -> CompletableFuture.supplyAsync(() -> additionalField(n, userId), Executors.newCachedThreadPool())).toList();

 CompletableFuture<Void> allTasks = CompletableFuture.allOf(taskList.toArray(new CompletableFuture[0]));
 // allOf 后需要 then 一下，我们是要返回结果的，所以是thenApply，所以有个入参v（没啥用，就是接口硬性要有）
 CompletableFuture<List<OrderManageRespVO>> allResults = allTasks.thenApply(v -> taskList.stream().map(CompletableFuture::join).toList());
 log.info("结束CompletableFuture");
 try {
     records = allResults.get();
 } catch (InterruptedException | ExecutionException e) {
     log.error("获取异步结果出错");
     throw exception(new ErrorCode(5000, "请联系管理员"));
 }

 private OrderManageRespVO additionalField(OrderManageRespVO respVO, Long userId) {
        log.info("开始调用方法additionalField");
```
