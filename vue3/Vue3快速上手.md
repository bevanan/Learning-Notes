...
```
其中`console.log(name)`在页面上得到的值是一个RefImpl对象，其中带下划线的都不是给我们用的数据`_value`
...
<script lang="ts" setup name="Person">
  ...
  // 这句代码的意思是，从person里面获取name, gender变量，注意，这并不是响应是的，需要通过以下方式转换
  let {name,gender} =  person
  // 通过toRefs将person对象中的n个属性批量取出，且依然保持响应式的能力
  let {name,gender} =  toRefs(person)
  // 通过toRef将person对象中的gender属性取出，且依然保持响应式的能力
  let age = toRef(person,'age')
  // 注意，这么取出来的参数，变动了值，是会影响到原有person的
  // 视频里老师的一句话是说，就是解构出来，前提是用reactive定义的响应是对象，用ref是不行的
...
<script setup lang="ts" name="App">
  ...
  let fullName = computed({
    // 读取
    get(){
      return firs tName.value + '-' + lastName.value
    },
    // 修改
    set(val){
...
<script lang="ts" setup name="Person">
  ...
  /* 
    ...
    watch的第一个参数是：被监视的数据
    watch的第二个参数是：监视的回调
    watch的第三个参数是：配置对象{deep(可监视对象内的值) 、immediate(监视器变成 do...while 模式)等等.....} 
  */
...
<router-link :to="{name:'xiang'}">跳转</router-link>
```
...
设置`props:true`后，在对应的Detail里面可直接使用`defineProps(['id', 'title', 'content'])`
...
// LoveTalk 从浏览器缓存中获取talk，如果是null，就是空数组
state(){
    return {
...
1. **`let {data:{content:title}} = await axios.get('https://api.uomg.com/api/rand.qinghua?format=json')`**
   - 返回的结果是一个对象，`data` 是该对象的一个属性。
   - 通过解构赋值，从 `data` 中提取出 `content` 属性，并将其重命名为 `title`。
2. **`let obj = {id:nanoid(), title}`**
   - 创建一个新的对象`obj`，其中包含两个属性：
     - `id`: 通过调用 `nanoid()` 生成一个唯一的ID。
     - `title`: 之前从API中获取到的 `content`（现在被称为 `title`）。
3. **`talkList.unshift(obj)`**
   - 将新创建的 `obj` 插入到数组 `talkList` 的开头。`unshift` 方法会把元素添加到数组的开始位置。
...
> 上下两个很像啊，触发方式；只不过下面这个是放在一个公共的ts内，要自己绑定事件关系。
...
> 就是通过一个公共的ts，里面有绑定了一些事件，通过触发事件进行数据的传输

**举例：**接收数据的组件中：绑定事件、同时在销毁前解绑事件：
...
> 父组件无法用到子组件的数据，已下未解决方案 v-slot; params为子组件打包过来的数据对象
